  ...
  <div @keydown="onKeydown">
    ...
    >
      <e-columns>
        ...
        <e-column field="ban_desc" header-text="Descrizione" text-align="Left" width="250"></e-column>
        <e-column field="ban_indirizzo" header-text="Indirizzo" text-align="Left" width="180"></e-column>
        <e-column field="ban_citta" header-text="Città" text-align="Left" width="140"></e-column>
        <e-column field="ban_iban" header-text="IBAN" text-align="Left" width="240"></e-column>
      </e-columns>
    </ejs-grid>
  </div>
</template>
...
import { Sort, Edit, Toolbar, Page, ExcelExport, PdfExport } from '@syncfusion/ej2-vue-grids'
...
const BancheEdit = () =>
  import(/* webpackChunkName: "tblcomuni" */ '@/components/tblcomuni/Banche/BancheEdit')
  ...
  methods: {
    ...
    onElimina() {
      ...
      if (sel_rows.length > 0) {
        let url = userService.getApipath() + 'banche/delete/' + sel_rows[0].ban_codice
        rsaUtility.consoleLog('BancheGrid onElimina url ' + url + '\n Row to be deleted: ', sel_rows[0])
        this.loaderOn()
        this.clear()
        axios
    ...
    onToolbarClicked(args) {
      ...
      if (args.item.id === 'Modifica') {
        ...
        if (sel_rows.length > 0) {
          ...
          this.indexEditRow = this.$refs.grid.ej2Instances.getSelectedRowIndexes()[0]
          this.$bvModal.show('banche-edit')
        } else
          DialogUtility.alert({
            title: 'Attenzione!',
    ...
    onKeydown(args) {
      if (args.key === 'F2') this.onToolbarClicked({ item: { id: 'Seleziona' } })
      if (args.key === 'F3') this.onToolbarClicked({ item: { id: 'Aggiungi' } })
      if (args.key === 'F4') this.onToolbarClicked({ item: { id: 'Modifica' } })
    ...
    addRecord(args) {
      ...
      this.data = this.$refs.grid.ej2Instances.getDataModule().dataManager.dataSource.json
