...
function GetStoredNumDoc(num, reg) {
  if (typeof reg !== 'string' && typeof num !== 'number') throw 'Almeno uno dei Parametri non è valido!'
  if (reg.length() < 1 || reg.length > 2) throw 'Lunghezza parametro reg non valida'
  ...
  if (reg.length() > 1) {
    numero += GAP_INTERNO + (reg.charCodeAt(1) - str.charCodeAt(0)) * GAP_INTERNO
  }
  return numero
}
...
function onDblClickOnComponent(_refs, component, suffix = '', disabled = false) {
  if (disabled) {
    consoleLog(`- Double Click is Disabled!`)
    return
  }
  consoleLog(`- Double Click on ${component} Component`)
  if (component.substring(0, 7) === 'tabelle') _refs[component + suffix].$refs.tabelleSearch.show()
...
function setGridHeight(grid, search) {
  ...
  if (Array.isArray(grid)) {
    ...
    if (!search && grid.length >= 15)
      return `${Math.round(window.innerHeight - (window.innerHeight * 30) / 100)}`
  } else {
    if (!grid.result) return dimensionConsts.HEIGHTGRIDWRAPPER
    if (search && grid.result.length >= dimensionConsts.SEARCHGRID) return '100%'
...
export const documenti = {
  0: { title: 'Fatture', label: 'Fattura', fromLabel: 'Scontrino', endpoints: 404 },
  1: {
